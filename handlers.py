...
async def _store_image_bytes(
    ctx, raw: bytes, package_id: str, role: str, version: str, *, fallback_url: str = "",
) -> tuple[str, str]:
    """Persist generated bytes in Imperal storage before a provider URL expires.

    The live storage gateway returns a stable public URL. `fallback_url` only
    protects callers in environments whose storage implementation has no URL
    (such as the in-memory test client), so a valid image is never replaced
    with an empty link.
    """
    image_format, _ = image_dims.describe_image(raw)
    extension = {"JPEG": "jpg", "PNG": "png", "WEBP": "webp"}.get(image_format, "bin")
    path = f"media-studio/{package_id}/{role}/{version}.{extension}"
...
async def _maybe_upscale_asset_image(ctx, api_key: str, image_url: str, *, package_id: str = "", role: str = "") -> dict:
    ...
    if package_id and role:
        try:
            stored_url, stored_path = await _store_image_bytes(
                ctx, original_bytes, package_id, role, "original", fallback_url=image_url,
            )
            result.update(image_url=stored_url, original_image_url=stored_url,
                          original_storage_path=stored_path)
        except Exception:
            await ctx.log(
                "Could not preserve the generated image in Media Hub storage; "
                "keeping the provider link for now.", level="warning",
            )
    if dims is None:
        await ctx.log(
    ...
    if package_id and role:
        try:
            final_url, stored_path = await _store_image_bytes(
                ctx, upscaled_bytes, package_id, role, "upscaled", fallback_url=upscaled_url,
            )
        except Exception:
            await ctx.log(
                "Could not preserve the upscaled image in Media Hub storage; "
                "keeping the provider link for now.", level="warning",
            )
    result.update(
        image_url=final_url,
        upscaled_image_url=final_url,
        upscaled_storage_path=stored_path,
...
def _package_to_entity(row: dict) -> MediaPackage:
    assets = [
        MediaAsset(
            ...
            original_storage_path=a.get("original_storage_path", ""),
            ...
            upscaled_storage_path=a.get("upscaled_storage_path", ""),
    ...
    async def work() -> ActionResult:
        ...
        for i, asset in enumerate(assets):
            ...
            try:
                ...
                asset.update(await _maybe_upscale_asset_image(
                    ctx, api_key, generated_url,
                    package_id=params.package_id, role=asset.get("role", ""),
                ))
                asset["status"] = "ready"
                asset["error"] = ""
...
async def regenerate_asset(ctx, params: RegenerateAssetParams) -> ActionResult:
    ...
    async def work() -> ActionResult:
        try:
            ...
            target.update(await _maybe_upscale_asset_image(
                ctx, api_key, generated_url,
                package_id=params.package_id, role=target.get("role", ""),
            ))
            target["status"] = "ready"
            target["error"] = ""
        except mc.ProviderError as exc:
...
async def generate_asset_upscale(ctx, params: GenerateAssetUpscaleParams) -> ActionResult:
    ...
    async def work() -> ActionResult:
        try:
            ...
            upscaled_format, upscaled_dimensions = image_dims.describe_image(upscaled_bytes)
            original_format, original_dimensions = image_dims.describe_image(original_bytes)
            stored_upscaled_url, upscaled_storage_path = await _store_image_bytes(
                ctx, upscaled_bytes, params.package_id, params.role, "upscaled", fallback_url=upscaled_url,
            )
            target.update(
                image_url=stored_upscaled_url,
                ...
                upscaled_image_url=stored_upscaled_url,
                upscaled_storage_path=upscaled_storage_path,
                upscaled_format=upscaled_format,
                upscaled_dimensions=upscaled_dimensions,
                upscaled_file_size=image_dims.format_file_size(len(upscaled_bytes)),
            )
    ...
    for row in rows:
        ...
        for asset in assets:
            ...
            for task_id in recovery.task_ids_from_asset(asset):
                ...
                if source_url:
                    break
            source_url = source_url or matches.get(role, "")
            if not source_url:
                unavailable += 1
                continue
            try:
...
async def delete_asset_image(ctx, params: DeleteAssetImageParams) -> ActionResult:
    """Delete precisely one permanently stored image version from an asset."""
    if params.version not in {"original", "upscaled"}:
        return _error("Choose either 'original' or 'upscaled' image to delete.", c.MEDIA_ASSET_NOT_FOUND)
    row = await st.get_package(ctx, params.package_id)
    if row is None:
        return _error(f"No media package found with id '{params.package_id}'.", c.MEDIA_PACKAGE_NOT_FOUND)
    assets = list(row.get("assets", []))
    target = next((asset for asset in assets if asset.get("role") == params.role), None)
    if target is None:
        return _error(f"No asset '{params.role}' in package '{params.package_id}'.", c.MEDIA_ASSET_NOT_FOUND)

    prefix = "original" if params.version == "original" else "upscaled"
    path = target.get(f"{prefix}_storage_path", "")
    if path:
        await ctx.storage.delete(path)
    target[f"{prefix}_storage_path"] = ""
    target[f"{prefix}_image_url"] = ""
    target[f"{prefix}_dimensions"] = ""
    target[f"{prefix}_format"] = ""
    target[f"{prefix}_file_size"] = ""
    if params.version == "upscaled":
        target["image_url"] = target.get("original_image_url", "")
    else:
        target["image_url"] = target.get("upscaled_image_url", "")
    if not target.get("image_url"):
        target["status"] = "pending"
    current = await st.update_package(ctx, params.package_id, {"assets": assets})
    return ActionResult.success(
        _package_to_entity(current),
        f"Deleted the {params.version} image for '{params.role}'.",
    )
